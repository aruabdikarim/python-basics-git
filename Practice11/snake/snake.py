...
while True:
    ...
    # 1. Счёт
    score_txt = font.render(f"Счёт: {score}", True, (90, 230, 140))
    screen.blit(score_txt, (16, panel_y))

    # 2. Рекорд (чуть подвинул вправо, чтобы не наезжало)
    hi_txt = font.render(f"Рекорд: {hi}", True, (150, 170, 150))
    screen.blit(hi_txt, (180, panel_y))
    
    # 3. Таймер еды
    time_left = max(0, (food["timer"] - (now - food["spawn_time"])) // 1000)
    timer_txt = font.render(f"Еда исчезнет через: {time_left}с", True, (255, 100, 100))
    
    # Чтобы таймер всегда был прижат к правому краю, вычитаем его ширину из ширины окна
    timer_x = W - timer_txt.get_width() - 20
    screen.blit(timer_txt, (timer_x, panel_y))
    # Grid
    for c in range(COLS):
        for r in range(ROWS):
   ...
   # Game Over
    if not alive:
        ...
        # 1. Отрисовка "GAME OVER"
        text_game_over = font_big.render("GAME OVER", True, (230, 70, 70))
        # Создаем прямоугольник текста и ставим его центр в центр экрана
        rect_game_over = text_game_over.get_rect(center=(W // 2, H // 2 - 30))
        screen.blit(text_game_over, rect_game_over)

        # 2. Отрисовка счета и кнопки рестарта
        text_retry = font.render(f"Счёт: {score}  |  Нажми R", True, (200, 200, 200))
        # Ставим центр этого текста чуть ниже первого
        rect_retry = text_retry.get_rect(center=(W // 2, H // 2 + 40))
        screen.blit(text_retry, rect_retry)
